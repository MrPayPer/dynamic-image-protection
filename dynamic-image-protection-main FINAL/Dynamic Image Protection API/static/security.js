...
// 4️⃣ Block Screenshot Attempts using keyup as in your snippet

document.addEventListener("keydown", function(event) {
    // Block Ctrl+Shift+I (Developer Tools)
    if (event.ctrlKey && event.shiftKey && (event.code === 'KeyI' || event.key === 'i' || event.keyCode === 73)) {
        blockScreenshot();
        event.preventDefault();
    ...
    // For Windows Snipping Tool (Win+Shift+S)
    if ((event.metaKey || event.keyCode === 91 || event.key === "Meta") && 
        event.shiftKey && 
        (event.code === 'KeyS' || event.key === 's' || event.keyCode === 83)) {
        blockScreenshot();
        event.preventDefault();
        event.stopPropagation();
        return false;
...
// Also try to catch copy events which might be used with screenshots
...
// Function for other blocking methods
function blockScreenshot() {
    ...
    // Move the alert to a timeout so the overlay appears immediately
    setTimeout(() => {
        alert("Screenshots and developer tools are temporarily disabled.");
    }, 10);
    
    // Clear clipboard
    navigator.clipboard.writeText("")
        .catch(err => console.log("Clipboard clear failed:", err));
    
    // Remove overlay after 1.5 seconds to match your snippet
    setTimeout(() => {
